...
public class BlockHelper {
	 ...
	 */
	public static void saveBlockPos(NbtCompound nbt, BlockPos pos) {
	 ...
	 */
	public static void saveBlockPos(NbtCompound nbt, BlockPos pos, String extra) {
		nbt.putInt(extra + "_X", pos.getX());
		nbt.putInt(extra + "_Y", pos.getY());
		nbt.putInt(extra + "_Z", pos.getZ());
	}
		...
		return IntStream.iterate((int) Math.floor(bb.minX), i -> i < Math.ceil(bb.maxX) + 16, i -> i + 16)
				...
				.flatMap(i ->
						...
						IntStream.iterate((int) Math.floor(bb.minZ), j -> j < Math.ceil(bb.maxZ) + 16, j -> j + 16)
								.boxed().flatMap(j -> {
									// get chunk access
									Chunk c = world.getWorldChunk(new BlockPos(i, 0, j));
